...
import nuclei3.media.utils.CarHelper;
...
public class MediaService extends MediaBrowserServiceCompat implements
    ...
    final private SessionManagerListener<CastSession> mSessionManagerListener = new SessionManagerListener<CastSession>() {
        ...
        private void onApplicationConnected(CastSession castSession) {
            ...
            if (mSession != null && mPlaybackManager != null) {
                ...
                final String deviceName = castSession.getCastDevice().getFriendlyName();
    ...
    public void onCreate() {
        ...
        if (casting) {
            mSessionExtras.putString(EXTRA_CONNECTED_CAST, mCastSession.getCastDevice().getFriendlyName());
            mMediaRouter.setMediaSessionCompat(mSession);
    ...
    @Override
    public void onCompletion(boolean autoContinue) {
        Playback playback = mPlaybackManager.getPlayback();
        if (playback != null) {
            MediaId id = playback.getCurrentMediaId();
            MediaProvider.getInstance().onPlaybackCompletion(playback, id, autoContinue);
